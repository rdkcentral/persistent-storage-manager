...
// Test case for PsmFloCreate
TEST_F(CcspPsmTestFixture, Create) {
    ...
    ANSC_HANDLE result = PsmFloCreate(hContainerContext, hOwnerContext, hAnscReserved);
...
// Test case for PsmFloRemove
TEST_F(CcspPsmTestFixture, Remove) {
    ...
    ANSC_STATUS status = PsmFloRemove(hObject);
    ASSERT_EQ(status, ANSC_STATUS_SUCCESS);
}

// Test case for PsmFloEnrollObjects
TEST_F(CcspPsmTestFixture, EnrollObjects) {
    ...
    ANSC_STATUS status = PsmFloEnrollObjects(hObject);
    ASSERT_EQ(status, ANSC_STATUS_SUCCESS);

    PsmFloRemove(hObject);
}

// Test case for PsmFloInitialize
TEST_F(CcspPsmTestFixture, Initialize) {
    ...
    ANSC_STATUS status = PsmFloInitialize(hObject);
    ASSERT_EQ(status, ANSC_STATUS_SUCCESS);
