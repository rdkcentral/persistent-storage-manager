...
log "INFO" "Running make for PsmSsp_gtest.bin..."
...
if [ -f "./source/test/PsmSsp_gtest.bin" ]; then
    log "INFO" "Running PsmSsp_gtest.bin"
    ./source/test/PsmSsp_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "PsmSsp_gtest.bin does not exist, cannot run tests"
    exit 1
fi
